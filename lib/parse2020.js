...
function parseCollection(collection) {
    ...
    collection.map( (doc) => {
        ...
        let bolsa = getBolsaData(bolsas, pais, orgID);

        if(isArea(laundry.simpleName(pais), areas_index) < 0) {
            let area = createArea(pais, row.Pais, metadata);
            areas.push(area);
        ...
        let org = null;
        if(isOrg(orgID + '-' + pais, orgs_index) < 0) {
            org = createOrg(row, metadata);
            orgs.push(org);
            orgs_index.push(org.id + '-' + pais);
        }
        else {
            org = orgs[isOrg(orgID + '-' + pais, orgs_index)];
        }

        let person = null;
        if(isPerson(personID + '-' + pais, persons_index) < 0) {
            person = createPerson(row, metadata);
            persons.push(person);
            persons_index.push(person.id + '-' + pais);
        }
        else {
            person = persons[isPerson(personID + '-' + pais, persons_index)];
        }
...
function createMember(id, row, metadata) {
    let clean_name = cleanPersonName(row.Persona);
    let membership = {
        id: id,
        role: row.Rol,
        ...
        parent_id: laundry.simpleName(laundry.launder(clean_name)),
