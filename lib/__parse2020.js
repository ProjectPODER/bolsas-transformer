...
function parseCollection(collection) {
    ...
    collection.map( (doc) => {
        ...
        let org = null;
        if(isOrg(laundry.simpleName(laundry.launder(row.Empresa)), orgs_index) < 0) {
            org = createOrg(row, metadata);
            orgs.push(org);
            orgs_index.push(org.id);
        }
        else {
            org = orgs[isOrg(laundry.simpleName(laundry.launder(row.Empresa)), orgs_index)];
        }

        let person = null;
        let person_name = cleanPersonName(row.CONSEJERE_NOMBRE);
        if(isPerson(laundry.simpleName(laundry.launder(row.CONSEJERE_NOMBRE)), persons_index) < 0) {
            person = createPerson(row, metadata);
            persons.push(person);
            persons_index.push(person.id);
        }
        else {
            person = persons[isPerson(laundry.simpleName(laundry.launder(row.CONSEJERE_NOMBRE)), persons_index)];
        }

        let memberID = laundry.simpleName(laundry.launder(row.Empresa)) + '_' + laundry.simpleName(laundry.launder(row.CONSEJERE_NOMBRE)) + '-bm';
...
function createOrg(row, metadata) {
    ...
    if(row.EMPRESA_SIGLAS && row.EMPRESA_SIGLAS != 'NA') {
        org.other_names = [{ name: row.EMPRESA_SIGLAS }]
    }
    if(row.EMPRESA_ID && row.EMPRESA_ID != 'NA') {
        org.identifiers = [{identifier:row.EMPRESA_ID}]
    }
    if(row.EMPRESA_SECTOR && row.EMPRESA_SECTOR != 'NA') org.subclassification = row.EMPRESA_SECTOR;
    if(row.EMPRESA_ACTIVIDAD && row.EMPRESA_ACTIVIDAD != 'NA') org.description = row.EMPRESA_ACTIVIDAD;
    if(row.EMPRESA_DIRECCION && row.EMPRESA_DIRECCION != 'NA') {
        if(!org.hasOwnProperty('contact_details')) org.contact_details = [];
        org.contact_details.push({
            "type": "office",
            "label": "Primary address",
            "value": row.EMPRESA_DIRECCION
        });
    }
    if(row.EMPRESA_FECHA_CONSTITUCION && row.EMPRESA_FECHA_CONSTITUCION != 'NA') org.founding_date = row.EMPRESA_FECHA_CONSTITUCION;
    if(row.EMPRESA_TELEFONOS && row.EMPRESA_TELEFONOS != 'NA') {
        if(!org.hasOwnProperty('contact_details')) org.contact_details = [];
        org.contact_details.push({
            "type": "voice",
    ...
    if(row.EMPRESA_URL && row.EMPRESA_URL != 'NA') org.links = [{id:row.EMPRESA_URL}];
...
function createMember(id, row, metadata) {
    let membership = {
        ...
        parent_id: laundry.simpleName(laundry.launder(row.CONSEJERE_NOMBRE)),
        parent_name: row.CONSEJERE_NOMBRE,
        parent_class: 'person',
        title: row.CONSEJERE_PUESTO
    };
    if(row.CONSEJERE_FECHA_INICIO && row.CONSEJERE_FECHA_INICIO != 'NA') membership.start_date = row.CONSEJERE_FECHA_INICIO;
    if(row.CONSEJERE_FECHA_FIN && row.CONSEJERE_FECHA_FIN != 'NA') membership.end_date = row.CONSEJERE_FECHA_FIN;
...
function createPerson(row, metadata) {
    let country = getCountryCode(row.PAIS);
    let person = {
        "id": laundry.simpleName(laundry.launder(row.CONSEJERE_NOMBRE)),
        "name": row.CONSEJERE_NOMBRE,
    ...
    if(row.CONSEJERE_ID && row.CONSEJERE_ID != 'NA') person.identifiers = [{identifier:row.CONSEJERE_ID}];
